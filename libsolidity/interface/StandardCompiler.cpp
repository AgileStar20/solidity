...
boost::optional<Json::Value> checkKeys(Json::Value const& _input, set<string> const& _keys, const string& _name )
{
...
boost::optional<Json::Value> checkSourceKeys(Json::Value const& _input, const string& _name)
{
...
boost::optional<Json::Value> checkOutputSelection(Json::Value const& _outputSelection)
{
	...
	for (auto const& sourceName: _outputSelection.getMemberNames())
	{
		...
		if (!sourceVal.isObject())
			return formatFatalError("JSONError",
